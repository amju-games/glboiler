...
aabb* aabb::transform_by(const mat4& m) const
{
  ...
  aabb* a(new aabb(
    mult(m, vec3(m_min.x, m_min.y, m_min.z)),
    mult(m, vec3(m_min.x, m_min.y, m_max.z))
    ));
  a->include(mult(m, vec3(m_min.x, m_max.y, m_min.z)));
  a->include(mult(m, vec3(m_min.x, m_max.y, m_max.z)));
  a->include(mult(m, vec3(m_max.x, m_min.y, m_min.z)));
  a->include(mult(m, vec3(m_max.x, m_min.y, m_max.z)));
  a->include(mult(m, vec3(m_max.x, m_max.y, m_min.z)));
  a->include(mult(m, vec3(m_max.x, m_max.y, m_max.z)));
  return a;
}
